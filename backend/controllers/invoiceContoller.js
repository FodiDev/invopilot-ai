...
// Generate a unigue number to avoid collision in the DB for invoice number
...
export async function getInvoiceById(req, res) {
  try {
    ...
    const { id } = req.params;
    let inv;
    if (isOblectIdString(id)) inv = await Invoice.findById(id);
...
export async function updateInvoice(req, res) {
  try {
    ...
    // if user changes the invoice number, the function checks if it already exists or not
    /* UPDATE */
    if (
      body.invoiceNumber &&
